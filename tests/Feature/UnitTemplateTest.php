...
class UnitTemplateTest extends TestCase
{
    ...
    /**
     * **PR-UOM-1 — تشديدٌ مقصود.** كانت هذه الحالة تسمح بتغيير المعامل بعد
     * الترحيل معتمدةً على أن السطر المرحَّل نسخ معامله فلا يتأثّر بأثرٍ رجعي.
     * العقد الجديد أشدّ: **التعديل نفسه يُرفض** بمجرد وجود أثرٍ مخزني على أي
     * منتج يستعمل القالب — لا يُترك احتمال قراءةٍ مستقبلية للقالب (تقرير،
     * شاشة تحرير) تُظهر معاملاً غير ما استُعمل فعلياً وقت الشراء.
     *
     * @test
     */
    public function changing_a_factor_after_a_posted_purchase_is_rejected_fail_closed(): void
    {
        $template = $this->template();
        $product = $this->product($template['id']);
        $this->buy($product['id'], 2, 50000, 'طبلية')->assertOk();
        ...
        $this->withToken($this->token)->putJson("/api/unit-templates/{$template['id']}", [
            ...
            'units' => [['name' => 'طبلية', 'factor' => 40]],   // ٥٠ ⇐ ٤٠ مرفوض
        ])->assertStatus(422);

        // لا أثر جزئي: لا القالب تغيّر ولا المستند القديم تأثّر بأي شكل.
        ...
        // والشراء الجديد يستمرّ بالمعامل الأصلي — لم يتغيّر شيء.
        $this->buy($product['id'], 1, 50000, 'طبلية')->assertOk();
        $this->assertSame(150, Product::findOrFail($product['id'])->quantity_on_hand);
    }

    /** الإضافة الآمنة (وحدة جديدة بلا تعارض) تبقى ممكنة رغم وجود أثرٍ مخزني. */
    /** @test */
    public function adding_a_new_unit_remains_allowed_after_a_posted_purchase(): void
    {
        $template = $this->template();
        $product = $this->product($template['id']);
        $this->buy($product['id'], 2, 50000, 'طبلية')->assertOk();
        ...
        $this->assertSame(
            10,
            UnitTemplateUnit::where('unit_template_id', $template['id'])->where('name', 'كرتون')->value('factor')
        );
    }
