...
class ImportJobInventoryOpeningApplyTest extends TestCase
{
    ...
    public function a_concurrent_apply_attempt_is_blocked_by_a_real_row_lock(): void
    {
        ...
        // — فتخطّي الحدث هنا آمنٌ ولا يغيّر ما يفحصه الاختبار. `withoutEvents()`
        // يعطّل أحداث النموذج كلها، بما فيها `BelongsToTenant` (تعبئة `tenant_id`
        // من السياق) — فنعوّضها صراحةً هنا. توليد `id` عبر `HasUuids` ليس حدثاً
        // بل جزءٌ من `Model::performInsert()` نفسه، فيبقى يعمل بلا تأثّر.
