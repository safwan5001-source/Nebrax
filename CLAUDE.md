...
**المرحلة الثانية (P1 — التمكين المحروس) مبنية:** `App\Services\TenantApplicationService`
...
بصلاحيتَي `apps.view`/`apps.manage` (owner/admin افتراضياً). **صراحة خارج هذه المرحلة:**
واجهة `web/`، وإنفاذ الاستحقاق على أي مسار API موجود مسبقاً، وحالات "يحتاج تهيئة"/"معلّق
للقراءة فقط" — تلك مراحل لاحقة منفصلة عمداً (P2).
...
**محدَّثة بعد تحقّق مباشر من `web/` — الملاحظة السابقة عن شاشتي المشتريات والمرتجعات
لم تعد صحيحة**، فقد بُنيتا بالكامل (مجموعة المشتريات في الشريط الجانبي مكتملة بعشرة
أقسام: طلب شراء → RFQ → عرض سعر → أمر شراء → فاتورة مشتريات → مرتجع → إشعار مدين،
والمرتجعات `/returns` و`/purchase-returns` قائمتان وتفاصيلهما مبنية) عبر جلسات موازية
لم تُحدَّث هذه الوثيقة بعدها.

**فجوة فعلية موجودة اليوم بدل ذلك:** أربعة روابط في مجموعة `hr` بالشريط الجانبي
(`/attendance`، `/payroll`، `/contracts`، `/requests`) تُعرَض بشارة "قريباً" وتؤدي
لصفحات **غير موجودة فعلياً (404)** — رغم أن وظائفها الأربع مبنية بالكامل ومتاحة
فعلاً كتبويبات داخل `/hr` نفسها (`RequestsManager`، تبويب `attendance`، تبويب `runs`
لمسيّر الرواتب) وكأقسام تفصيلية لكل موظف في `/hr/employees/[id]` (`contracts`،
`attendance`، `payroll`، `leave`، `requests`، `attachments`). الإصلاح صغير ومحدَّد:
توجيه الروابط الأربعة إلى `/hr` (مع دعم اختيار التبويب عبر معامل رابط، إذ لا تقرأ
`/hr/page.tsx` اليوم أي `?tab=` عند التحميل) ووسمها `built: true`.

بعدها: اختبارات الواجهة، الإعدادات/الاشتراك، تصدير Excel/PDF، وZATCA المرحلة 2 الكاملة
(عند توفّر الشهادات).
