...
- **مفاتيح مكرَّرة في `web/src/messages/ar.json`.** داخل `superGroups` تتكرّر حرفياً مفاتيح منها
  `return_window_days` و`require_return_source` وتلميحاتها. لا أثر سلوكياً — `JSON.parse` يأخذ
  الأخيرة — لكنها تُخفي أيّ القيمتين هي المعروضة فعلاً، ويصعب معها تتبّع تغيير النصّ.
  **التنظيف لا يُدمج مع تغييرٍ وظيفي:** إعادة كتابة الملف تُعيد المحاذاة فتبتلع ضجّةُ الفرق
  المراجعةَ الحقيقية. يستحق PR منفصلاً لا يفعل غير هذا، ويشمل `en.json` بالفحص نفسه.
