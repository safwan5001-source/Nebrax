...
- REST API كامل (Sanctum + RBAC بالأدوار + عزل tenant) + طبقة الاشتراكات وحدود الخطط وتحصين المصادقة. توجد كذلك مساحة تشغيل داخلية للمنصة (`/api/platform/*` و`/platform`) بحساب `PlatformAdministrator` مستقل؛ تعرض مؤشرات مجمّعة للقراءة فقط ولا تمرّ عبر `SetTenant`.
...
database/migrations/   000001..000080 (النواة + المستندات + المخزون + الاشتراكات + ZATCA + مديرو المنصة)
