...
class Settings
{
    ...
    public const DEFAULTS = [
        ...
        'sales' => [
            ...
            'invoice_prefix'       => 'INV',
            // بادئة سند التسليم العام؛ مستقلة عن الفاتورة لأنه دليل تشغيلي لا فاتورة.
        ...
        /**
         * ═══════════════════════════════════════════════════════════
         *  لغة مستندات AWJ الافتراضية للمؤسسة
         * ═══════════════════════════════════════════════════════════
         *  قرارٌ مستقلٌّ عن لغة واجهة أَوْج (`ui_locale`) وعن اختيار التصميم
         *  في PR #633. تُستهلَك عند إنشاء مسودة جديدة تحمل `language = null`،
         *  فتسقط قيمتها الفعلية إلى هذه القيمة، ثم إلى `ar` كخيار أخير عند
         *  غياب أي إعداد. لا تمسّ مستنداً قائماً مطلقاً — سواء مسودة أو مرحّلاً.
         *
         *  المفتاح المدعوم واحدٌ في V1: `default_language` من مفردات
         *  `PrintTemplateContract::DOCUMENT_LANGUAGES` (ar/en/bilingual)
         *  أو `null` = «اترك للنظام (العربية)».
         *
         *  **الافتراض `null`** = سلوك اليوم حرفياً (سقوط إلى `ar`)، فلا تتغيّر
         *  مخرجات مستأجر قائم بترقية صامتة.
         */
        'documents' => [
         ...
         * ═══════════════════════════════════════════════════════════
         *  انظر `App\Support\DocumentIntelligence`. القراءة عبر الخدمة
         *  `DocumentIntelligencePolicy` لا من هنا مباشرةً.
         *
         *  ── `processing_enabled` — هل تُشغَّل المعالجة الذكية (الاستخراج)؟
         *     **الافتراض `false`**: لا يُفعَّل الذكاء لمستأجر قائم أو جديد إلا
         *     بقرار صريح. توصيل مزود خارجي (Gemini وغيره) مرحلةٌ لاحقة مستقلّة.
         *
         *  ── `allowed_document_types` — الأنواع المسموح بمعالجتها ذكياً
         *     (قائمة صريحة من `DocumentTypeCatalog`). **الافتراض `[]`**: لا نوع
         *     يُعالَج ذكياً حتى يختاره المستأجر. لا يقيّد هذا رفع المستندات إلى
         *     المركز (المركز أرشيفٌ لكل الأنواع)، بل ما يستخرجه الذكاء وحده.
         *
         *  ── `retention_mode` — مصير الأصل بعد اكتمال المعالجة بأمان.
         *     **الافتراض `document_center_only`** = السلوك القائم حرفياً (يبقى
         *     في المركز، بلا إرفاق تلقائي، بلا حذف مبكّر). مستقلّ تماماً عن
         *     `processing_enabled`: تفعيل الذكاء لا يفرض الاحتفاظ ولا يمنعه.
         */
