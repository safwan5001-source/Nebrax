 ...
 *
 * **قرار نطاق صريح**: قد ينشئ التاجر أكثر من `BuilderApp` واحد (لا قيد
 * فريد على `builder_apps.tenant_id` — أداة تأليف حرّة، انظر
 * `BuilderAppController`)، فلا يوجد عمود "التطبيق النشط" اليوم. هذه النقطة
 * تُعرِّف "التجربة الحية" بأنها **أحدث نسخة منشورة عبر كل تطبيقات المستأجر
 * مجتمعة** (`ORDER BY published_at DESC`) — مطابقاً لسلوك التاجر المتوقَّع
 * (ينشر نسخةً فتصبح هي الحية) دون تعديل مخطط قاعدة البيانات أو ابتكار عمود
 * "نشط" جديد؛ عزلٌ تلقائي عبر `TenantScope` على `BuilderPublishedExperienceVersion`
 * نفسه، لا استعلاماً يدوياً يتجاوزه. قابلٌ للتغيير لاحقاً (عمود `is_live`
 * صريح إن دعت الحاجة) دون كسر شكل هذا العقد العام — يتغيّر الاستعلام فقط.
 *
