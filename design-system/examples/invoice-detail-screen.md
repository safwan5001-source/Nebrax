...
## 6. التصدير والمشاركة — `financial/financial-ui.md` §6

قالب `<InvoiceDocument>` = مستند A4 احترافي (فاتورة ضريبية) داخل `#print-root.print-only`
(مخفيّ على الشاشة). أربعة إجراءات في شريط الرأس (`no-print`):

| الإجراء | الأداة | السلوك |
|---|---|---|
| **طباعة / حفظ PDF** | `window.print()` + `@media print` | جودة عربية مثالية، نصّ قابل للتحديد (المسار الموصى للأرشفة) |
| **تنزيل PDF** | `lib/pdf.ts` (`downloadPdf`) | html2canvas → jsPDF → ملف `.pdf` (النص صورة — مقبول للإرسال) |
| **مشاركة** | `lib/pdf.ts` (`sharePdf`) | Web Share API يرسل الـ PDF **كملف** (واتساب/بريد)؛ تراجع للتنزيل عند عدم الدعم |
| **إكسل** | `lib/xlsx.ts` (`exportXlsx`) | SheetJS → ملف `.xlsx` حقيقي (بيانات + سطور بأرقام قابلة للجمع) |

- **الاستيراد ديناميكي:** المكتبات الثقيلة (`jspdf`/`html2canvas`/`xlsx`) تُستورَد داخل المعالِجات
  فتُقسَّم عن الحزمة الأساسية (صفحة التفاصيل ~6 kB فقط) — قاعدة أداء عامة للأدوات الثقيلة.
- **حالات الإجراء:** `busy` يعطّل الأزرار أثناء التوليد ويُظهر «جارٍ التوليد…»؛ نجاح/فشل عبر Toast؛
  إلغاء المستخدم للمشاركة (`AbortError`) لا يُعدّ خطأ.
- التقاط PDF: يُظهَر `#print-root` بتخطيط كامل خارج الشاشة (`left:-10000px`) أثناء الالتقاط ثم يُعاد.
