...
1. **On `2292c2c7`** (an earlier commit): a real, completed failure in `CustomerDigitalAccessTest` (`customer_identities_tenant_email_unique`) — added in PR #743, months before this branch, untouched by this diff. Genuinely pre-existing and unrelated.
2. **On `d31fc549`**: the check-run API reported `in_progress` for 40+ minutes (well past the 6-10 minutes every comparable job took), which I treated as a hang, cancelled, and re-ran. The re-run's check-run API *also* showed `in_progress` for an extended period, but when I pulled the job's actual logs directly, it had in fact **completed and failed** in ~9 minutes (`run_attempt: 2`, `started_at` 19:53:04, `completed_at` 20:02:13) — the check-run status API was stale/lagging, not the job itself. The real failure: this PR's own `StorefrontCatalogApiTest` fixture set `Product.discount_type => 'fixed'`, but that column is a genuine `enum(['percent', 'amount'])` at the database level (`2025_01_01_000003_create_partners_and_products.php`). SQLite does not enforce that CHECK constraint, so the invalid fixture passed silently in every local run; PostgreSQL correctly rejected it, and the aborted transaction then cascaded into unrelated-looking failures for other tests sharing the connection (`public_api_idem`, `invoices.zatca_icv`) — noise from the one poisoned transaction, not independent bugs.

**Fixed in `3602ccbc`** (`discount_type => 'amount'`) and re-verified locally (14/14 passing). This was a genuine bug in this PR's own test code, caught only by pgsql's stricter constraint enforcement — not a CI infrastructure issue, and I corrected the record on the PR accordingly after initially mischaracterizing it as one. Every job CI performs was run directly in this session before each push (`php artisan test` locally on sqlite — the only DB driver available here — Biome, `tsc --noEmit`, Vitest, plus manual verification of the fix against the actual pgsql failure signature from the CI logs). Confirm the final GitHub-side CI status at `https://github.com/safwan5001-source/Nebrax/pull/766` before merging.
