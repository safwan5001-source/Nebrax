...
A second review round on the fix commit (a81b908) found one more:

4. **P1 — the shared store `ApiClient` bearer means one customer can exhaust the whole store's authentication quota.** `EnforcePublicApiRateLimit:sensitive` is keyed per `(tenant, ApiClient)` — correct for M2M/store-level throttling, but every mobile installation of one tenant's app authenticates with the *same* shared `ApiClient` bearer (that is the whole "store token" design, unrelated to which human is using the app). So the 10/min `sensitive` budget was, in effect, shared across every concurrent customer of that store: a handful of legitimate concurrent logins, or one caller holding the store token, could lock out everyone else trying to register/login/request-OTP/verify-OTP at the same time — and the authenticated `me`/`logout` group had the same shared-key problem under `write`. Fixed with four new named `RateLimiter::for()` limiters in `TenancyServiceProvider` (`commerce-customer-register`, `commerce-customer-login`, `commerce-customer-otp-request`, `commerce-customer-otp-verify` — IP+email/phone dual limits, mirroring `customer-register`/`customer-login`'s own pattern but keyed off `TenantContext` since `/commerce/v1` has no `tenantSlug` route parameter) applied per-route alongside the existing `EnforcePublicApiRateLimit:sensitive`, plus `commerce-customer-session` (keyed by the resolved `CustomerIdentity`, applied *after* `AuthenticateCommerceCustomer` in the authenticated `me`/`logout` group) so one customer's traffic cannot exhaust another's. The store-wide `EnforcePublicApiRateLimit` classes stay in place unchanged as the outer abuse-control layer; these are additive, per-customer dimensions, not a replacement. `otp/request` and `otp/verify` deliberately use two *separate* limiter names (not one shared "otp" bucket) so a customer's own request-then-retry-verify flow for one phone doesn't compete against itself. New regression test (`repeated_failed_logins_for_one_email_do_not_block_a_different_email`); full re-verification on both SQLite and PostgreSQL (652 passed/10 skipped on SQLite, 662 passed on PostgreSQL).
