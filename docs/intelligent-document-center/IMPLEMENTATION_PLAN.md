...
**Status: implemented — internal contract only.** PR-11 adds the vendor-neutral `DocumentSourceConnector`, immutable bounded envelope/receipt contracts, and an internal `web` connector. `DocumentChannelIdentity` binds a non-secret fingerprint and masked display reference to a trusted tenant/branch; a source receipt with database uniqueness on identity/channel/normalized-reference makes an exact replay return the original batch/file and makes different content a safe conflict. Receipt and audit evidence are branch-scoped and append-only. The receiver recomputes SHA-256 from inspected bytes, establishes tenant/branch from the identity only, and delegates to the existing secure intake, storage, quarantine, signed-download, and opt-in processing gates.
