...
**Status: implemented — internal contract only.** PR-11 adds the vendor-neutral `DocumentSourceConnector`, immutable bounded envelope/receipt contracts, and an internal `web` connector. Core canonicalization preserves case by default after trim/length validation; `web` explicitly declares lowercase semantics, while future channels stay case-sensitive until their contract says otherwise. `DocumentChannelIdentity` binds a non-secret fingerprint and masked display reference to a trusted tenant/branch; a source receipt with database uniqueness on identity/channel/channel-canonical-reference makes an exact replay return the original batch/file and makes different content a safe conflict. The reception transaction locks the identity and returns an existing receipt as a replay decision; replay/conflict audit is written once after commit so a conflicting delivery cannot lose evidence to rollback. Receipt and audit evidence are branch-scoped and append-only. The receiver recomputes SHA-256 from inspected bytes, establishes tenant/branch from the identity only, and delegates to the existing secure intake, storage, quarantine, signed-download, and opt-in processing gates.
