...
The validator consumes only integer minor-unit evidence and uses the existing invoice integer tax semantics: `intdiv` with explicit half-up behavior for inclusive and exclusive tax. It validates line and header totals, tax totals, missing currency, discount bounds, unsupported tax rates, quantities, and rounding differences. The explicit current tolerance is one minor unit; a difference beyond it is blocking. Evidence is never corrected silently.
