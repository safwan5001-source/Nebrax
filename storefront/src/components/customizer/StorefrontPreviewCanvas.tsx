        ...
        {(config.header.showCategoryNav || extraNav.length > 0) && !compact && (
          ...
          >
            <div
              className={cn(
                storeContainerClassName,
                "flex h-9 items-center gap-4 overflow-hidden text-sm",
              )}
            >
...
function SectionRule({ title, action }: { title: string; action: string }) {
  return (
    <div className="flex items-start justify-between gap-4">
      ...
      <span className="text-xs font-bold text-store-primary md:text-sm">
        {action}
      </span>
    </div>
  );
...
function FooterCol({
  ...
  return (
    <div>
      <h2 className="text-sm font-bold text-store-footer-foreground">
        {title}
      </h2>
