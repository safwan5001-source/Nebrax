...
import { formatLiters, litersToMilliliters } from '@/lib/fuel-quantity';
...
const emptyForm = { fuel_station_id: '', fuel_tank_id: '', fuel_product_id: '', warehouse_id: '', supplier_id: '', dispatched_liters: '', received_liters: '', received_total_cost: '', delivery_note_number: '' };
...
export default function FuelStationsReceivingPage() {
  ...
  const dispatchedMl = litersToMilliliters(form.dispatched_liters, true);
  const receivedMl = litersToMilliliters(form.received_liters, true);
  const varianceMl = dispatchedMl !== null && receivedMl !== null ? receivedMl - dispatchedMl : null;
  ...
  async function submit(event: React.FormEvent) {
    ...
    const costMinor = riyalToMinor(form.received_total_cost);
    if (!isValidRiyal(form.received_total_cost) || costMinor < 0 || dispatchedMl === null || receivedMl === null) return;
    setError(null); setSaving(true);
    try {
      await api('/fuel-stations/deliveries', { method: 'POST', body: {
        fuel_station_id: form.fuel_station_id, fuel_tank_id: form.fuel_tank_id, fuel_product_id: form.fuel_product_id, warehouse_id: form.warehouse_id, supplier_id: form.supplier_id,
        dispatched_liters: form.dispatched_liters, received_liters: form.received_liters, received_total_cost_minor: costMinor,
        delivery_note_number: form.delivery_note_number || null, idempotency_key: `web-receipt-${crypto.randomUUID()}`,
      } });
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(21rem,0.7fr)]">
      <Card><CardHeader><CardTitle>{t('deliveries')}</CardTitle></CardHeader><CardContent>{deliveries.length === 0 ? <p className="rounded-md border border-dashed border-border px-4 py-10 text-center text-sm text-muted">{t('emptyDeliveries')}</p> : <div className="overflow-x-auto"><table className="w-full min-w-[38rem] text-sm"><thead className="border-b border-border text-xs text-muted"><tr><th className="px-2 py-2 text-start">{t('deliveryNote')}</th><th className="px-2 py-2 text-start">{t('supplier')}</th><th className="px-2 py-2 text-start">{t('received')}</th><th className="px-2 py-2 text-start">{t('value')}</th><th className="px-2 py-2 text-start">{t('status')}</th></tr></thead><tbody>{deliveries.map((delivery) => <tr key={delivery.id} className="border-b border-border/70 last:border-0"><td className="num px-2 py-3 text-text">{delivery.delivery_note_number ?? '—'}</td><td className="px-2 py-3 text-text">{suppliers.find((supplier) => supplier.id === delivery.supplier_id)?.name ?? '—'}</td><td className="num px-2 py-3 text-text">{formatLiters(delivery.received_liters)}</td><td className="num px-2 py-3 text-text">{formatMinorRiyal(delivery.received_total_cost_minor)}</td><td className="px-2 py-3"><Badge tone={delivery.status === 'approved' ? 'positive' : 'muted'}>{t(`status_${delivery.status}` as 'status_draft' | 'status_approved')}</Badge></td></tr>)}</tbody></table></div>}</CardContent></Card>
      <Card><CardHeader><CardTitle>{t('newDelivery')}</CardTitle></CardHeader><CardContent><form onSubmit={submit} className="grid gap-3"><div className="space-y-1.5"><Label htmlFor="station">{t('station')}</Label><Select id="station" value={form.fuel_station_id} onChange={(event) => selectStation(event.target.value)} required><option value="">—</option>{stations.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.code}</option>)}</Select></div><div className="space-y-1.5"><Label htmlFor="warehouse">{t('warehouse')}</Label><Select id="warehouse" value={form.warehouse_id} onChange={(event) => set('warehouse_id', event.target.value)} required><option value="">—</option>{compatibleWarehouses.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.code}</option>)}</Select><p className="text-xs leading-relaxed text-muted">{t('warehouseHint')}</p></div><div className="space-y-1.5"><Label htmlFor="tank">{t('tank')}</Label><Select id="tank" value={form.fuel_tank_id} onChange={(event) => set('fuel_tank_id', event.target.value)} required><option value="">—</option>{compatibleTanks.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.code}</option>)}</Select></div><div className="space-y-1.5"><Label htmlFor="fuel-product">{t('fuelProduct')}</Label><Select id="fuel-product" value={form.fuel_product_id} onChange={(event) => set('fuel_product_id', event.target.value)} required><option value="">—</option>{fuelProducts.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.code}</option>)}</Select></div><div className="space-y-1.5"><Label htmlFor="supplier">{t('supplier')}</Label><Select id="supplier" value={form.supplier_id} onChange={(event) => set('supplier_id', event.target.value)} required><option value="">—</option>{suppliers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></div><div className="grid gap-3 sm:grid-cols-2"><VolumeField id="dispatched" label={t('dispatched')} value={form.dispatched_liters} onChange={(value) => set('dispatched_liters', value)} /><VolumeField id="received" label={t('received')} value={form.received_liters} onChange={(value) => set('received_liters', value)} /></div>{varianceMl !== null && <p className={`rounded-md px-3 py-2 text-sm ${varianceMl === 0 ? 'bg-primary-soft text-text' : 'bg-warning/10 text-text'}`}>{t('variancePreview', { value: formatLiters(Math.abs(varianceMl) / 1000), direction: varianceMl === 0 ? t('noVariance') : varianceMl > 0 ? t('surplus') : t('shortage') })}</p>}<div className="space-y-1.5"><Label htmlFor="value">{t('value')}</Label><Input id="value" inputMode="decimal" className="num text-end" value={form.received_total_cost} onChange={(event) => set('received_total_cost', event.target.value)} required aria-describedby="delivery-value-hint" /><p id="delivery-value-hint" className="text-xs text-muted">{t('moneyHint')}</p></div><div className="space-y-1.5"><Label htmlFor="note">{t('deliveryNote')}</Label><Input id="note" value={form.delivery_note_number} onChange={(event) => set('delivery_note_number', event.target.value)} /></div><Button type="submit" disabled={saving || !isValidRiyal(form.received_total_cost) || riyalToMinor(form.received_total_cost) < 0 || dispatchedMl === null || receivedMl === null}>{saving ? <Loader2 className="h-4 w-4 animate-spin" strokeWidth={1.7} /> : <Plus className="h-4 w-4" strokeWidth={1.7} />}{t('createDraft')}</Button></form></CardContent></Card>
    </div>
    <Card><CardHeader><CardTitle>{t('supplierInvoices')}</CardTitle></CardHeader><CardContent>{invoices.length === 0 ? <p className="text-sm text-muted">{t('emptyInvoices')}</p> : <div className="overflow-x-auto"><table className="w-full min-w-[36rem] text-sm"><thead className="border-b border-border text-xs text-muted"><tr><th className="px-2 py-2 text-start">{t('invoiceNumber')}</th><th className="px-2 py-2 text-start">{t('supplier')}</th><th className="px-2 py-2 text-start">{t('value')}</th><th className="px-2 py-2 text-start">{t('status')}</th></tr></thead><tbody>{invoices.map((invoice) => <tr key={invoice.id} className="border-b border-border/70 last:border-0"><td className="num px-2 py-3">{invoice.invoice_number}</td><td className="px-2 py-3">{suppliers.find((supplier) => supplier.id === invoice.supplier_id)?.name ?? '—'}</td><td className="num px-2 py-3">{formatMinorRiyal(invoice.total_value_minor)}</td><td className="px-2 py-3"><Badge tone={invoice.status === 'matched' ? 'positive' : invoice.status === 'value_variance_pending' ? 'warning' : 'muted'}>{t(`invoice_${invoice.status}` as 'invoice_unmatched' | 'invoice_partially_matched' | 'invoice_matched' | 'invoice_value_variance_pending')}</Badge></td></tr>)}</tbody></table></div>}</CardContent></Card>
  </div>;
}

function VolumeField({ id, label, value, onChange }: { id: string; label: string; value: string; onChange: (value: string) => void }) { return <div className="space-y-1.5"><Label htmlFor={id}>{label}</Label><Input id={id} inputMode="decimal" className="num text-end" value={value} onChange={(event) => onChange(event.target.value)} required aria-describedby={`${id}-hint`} /><p id={`${id}-hint`} className="text-xs text-muted">L</p></div>; }
