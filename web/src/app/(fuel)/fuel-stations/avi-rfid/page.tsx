...
export default function FuelAviRfidPage() {
  ...
  const tagColumns = useMemo<ColumnDef<Tag, unknown>[]>(() => [
    ...
    { accessorKey: 'effective_from', header: t('effectiveFrom'), cell: ({ row }) => <span className="num">{formatDate(row.original.effective_from)}</span> },
  ], [drivers, nameFor, partners, t, vehicles]);
  const authorizationColumns = useMemo<ColumnDef<Authorization, unknown>[]>(() => [
    { accessorKey: 'authorized_at', header: t('issuedAt'), cell: ({ row }) => <span className="num">{formatDateTime(row.original.authorized_at)}</span> },
