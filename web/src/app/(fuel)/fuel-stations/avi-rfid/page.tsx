...
export default function FuelAviRfidPage() {
  ...
  const authorizationColumns = useMemo<ColumnDef<Authorization, unknown>[]>(() => [
    { accessorKey: 'authorized_at', header: t('issuedAt'), cell: ({ row }) => <span className="num">{new Date(row.original.authorized_at).toLocaleString(DISPLAY_LOCALE)}</span> },
