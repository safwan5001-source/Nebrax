...
type Diagnostics = { runtime: { queue_configured: boolean; worker_online: boolean; scanner_ready: boolean; provider_configured: boolean; provider_network_locked: boolean; runs: { queued: number; running: number; failed: number } } };
...
export default function PlatformDocumentOperationsPage() {
  ...
  return <div className="min-h-screen bg-background px-4 py-6 text-text md:px-8"><main className="mx-auto max-w-7xl space-y-5"><header className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between"><div><Button asChild variant="ghost" size="sm"><Link href="/platform">←</Link></Button><h1 className="mt-2 text-xl font-semibold">{t('platformTitle')}</h1><p className="mt-1 text-sm text-muted">{t('platformSubtitle')}</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={busy === 'export'} onClick={() => void exportAudit()}><Download className="h-4 w-4" aria-hidden="true" />{t('exportAudit')}</Button><Button variant="outline" disabled={loading} onClick={() => void load()}><RefreshCw className="h-4 w-4" aria-hidden="true" />{t('refresh')}</Button></div></header>
    ...
    {loading ? <Card><CardContent className="py-10 text-sm text-muted">{t('loading')}</CardContent></Card> : error ? <Card><CardContent className="flex flex-wrap items-center gap-3 py-10 text-sm text-muted"><CircleAlert className="h-5 w-5" aria-hidden="true" />{error}<Button variant="outline" onClick={() => void load()}>{t('retry')}</Button></CardContent></Card> : overview && usage && diagnostics ? <>
      <section aria-label={t('runtime')} className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Card><CardContent className="py-4"><p className="text-xs text-muted">{t('runtime')}</p><Badge className="mt-2" tone={diagnostics.runtime.worker_online ? 'positive' : 'warning'}>{diagnostics.runtime.worker_online ? t('workerOnline') : t('workerOffline')}</Badge></CardContent></Card><Card><CardContent className="py-4"><p className="text-xs text-muted">{t('networkLocked')}</p><Badge className="mt-2" tone={diagnostics.runtime.provider_network_locked ? 'warning' : 'positive'}>{diagnostics.runtime.provider_network_locked ? t('networkLocked') : t('networkOpen')}</Badge></CardContent></Card><Card><CardContent className="py-4"><p className="text-xs text-muted">{t('processing')}</p><p className="mt-1 text-2xl font-semibold tabular-nums">{diagnostics.runtime.runs.running}</p></CardContent></Card><Card><CardContent className="py-4"><p className="text-xs text-muted">{t('summaryFailed')}</p><p className="mt-1 text-2xl font-semibold tabular-nums">{diagnostics.runtime.runs.failed}</p></CardContent></Card></section>
