...
export default function CustomerRefundDetailPage() {
  ...
  const load = useCallback(() => {
    setLoadError(null);
    api<{ data: CustomerRefund }>(`/customer-refunds/${params.id}`)
  ...
  async function act(action: 'post' | 'reverse') {
    setBusy(true);
    try {
      await api(`/customer-refunds/${params.id}/${action}`, { method: 'POST' });
      ...
      <Card>
        ...
        <CardContent>
          <dl className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2">
            <div>
              <dt className="text-xs text-muted">{t('customer')}</dt>
            ...
            <table className="w-full min-w-[24rem] text-sm">
              ...
              <tbody>
                {(refund.allocations ?? []).map((allocation) => {
                  const href = allocation.source_type === 'credit_note'
                    ? `/credit-notes/${allocation.source_id}`
                    : `/returns/${allocation.source_id}`;

                  return (
                    <tr key={allocation.id} className="border-b border-border/70 last:border-0">
                      <td className="px-2 py-2">
                        <Link href={href} className="num text-primary hover:underline">
                          {allocation.source_number ?? allocation.source_id}
                        </Link>
                        <span className="mx-2 text-xs text-muted">{t(`source_${allocation.source_type}`)}</span>
                      </td>
