...
export default function SupplierPaymentsPage() {
  ...
  const definitions = useMemo<FilterDefinition[]>(() => [
    {
      key: 'partner_id', label: tsp('supplier'), kind: 'entity', quick: true,
      searchPlaceholder: tsp('search'), emptyText: tsp('empty'),
      options: partnerList.map((partner) => ({ value: partner.id, label: partner.name })),
    },
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((payment) => {
      if (query) {
        const haystack = [payment.number, partners[payment.partner_id], payment.method, payment.status]
