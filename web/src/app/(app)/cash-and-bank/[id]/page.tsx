...
export default function CashBankDetailPage() {
  ...
  if (!item) return <div className="mx-auto max-w-5xl space-y-4"><p className="rounded-md bg-negative/10 px-4 py-3 text-sm text-negative">{t('load_failed')}</p><Link href="/cash-and-bank"><Button variant="outline">{t('back')}</Button></Link></div>;
  const Icon = item.type === 'bank' ? Landmark : Wallet;
  const actions = (mobile = false) => <>
    <Link className={mobile ? 'shrink-0' : undefined} href={`/cash-and-bank/new?edit=${item.id}`}><Button variant="outline"><Pencil className="h-4 w-4" strokeWidth={1.7} />{t('edit')}</Button></Link>
    ...
    <Link className={mobile ? 'shrink-0' : undefined} href={`/cash-and-bank/transfer?source=${item.id}`}><Button variant="outline"><ArrowLeftRight className="h-4 w-4" strokeWidth={1.7} />{t('transfer')}</Button></Link>
