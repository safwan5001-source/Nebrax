...
export default function NewRecurringPage() {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const created = await api<{ data: { id: string } }>('/recurring-invoices', {
        method: 'POST',
        body: {
          title: title || null,
      ...
      success(tc('created'));
      router.push(`/recurring-invoices/${created.data.id}`);
    } catch (err) {
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-1.5 lg:col-span-3">
              <Label htmlFor="title">{t('template')}</Label>
            ...
            <div className="space-y-1.5">
              ...
              <Input id="start" type="date" dir="ltr" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="end">{t('end_date')}</Label>
              <Input id="end" type="date" dir="ltr" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
            </div>
          </div>
        </CardContent>
      </Card>
