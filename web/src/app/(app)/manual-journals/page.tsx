...
export default function ManualJournalsPage() {
  ...
  const columns = useMemo<ColumnDef<ManualJournal, unknown>[]>(() => [
    ...
    { id: 'actions', header: t('actions'), cell: ({ row }) => {
      ...
      return <div className="flex justify-end gap-1"><Button asChild size="icon" variant="ghost" aria-label={t('view')}><Link href={`/manual-journals/${journal.id}`}><Eye className="h-4 w-4" strokeWidth={1.7} /></Link></Button>{isDraft ? <Button asChild size="icon" variant="ghost" aria-label={t('edit')}><Link href={`/manual-journals/new?edit=${journal.id}`}><Pencil className="h-4 w-4" strokeWidth={1.7} /></Link></Button> : <Button size="icon" variant="ghost" disabled title={t('draftOnly')} aria-label={t('edit')}><Pencil className="h-4 w-4" strokeWidth={1.7} /></Button>}<Button size="icon" variant="ghost" disabled={busy} onClick={() => duplicate(journal.id)} aria-label={t('duplicate')}><Copy className="h-4 w-4" strokeWidth={1.7} /></Button><Button size="icon" variant="ghost" disabled={!isDraft || busy} title={!isDraft ? t('draftOnly') : undefined} onClick={() => remove(journal.id)} aria-label={t('delete')}><Trash2 className="h-4 w-4 text-negative" strokeWidth={1.7} /></Button>{journal.status === 'posted' && <Button asChild size="icon" variant="ghost" aria-label={t('reverse')}><Link href={`/manual-journals/${journal.id}`}><RotateCcw className="h-4 w-4" strokeWidth={1.7} /></Link></Button>}</div>;
    } },
