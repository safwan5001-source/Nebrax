...
export default function ReturnsPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    Promise.all([
      api<{ data: ReturnDoc[] }>(`/returns?type=sales${branchViewQuery(view, true)}`),
  ...
  useEffect(() => {
    ...
    router.replace(url.toString() ? `/returns?${url.toString()}` : '/returns', { scroll: false });
  }, [explorer, router]);
  ...
  ], [customerOptions, statusOptions, t]);
