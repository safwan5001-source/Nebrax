    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="no-print ms-auto flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => void handleXlsx()} disabled={!!busy}>
            <FileSpreadsheet className="h-4 w-4" strokeWidth={1.7} />
            {busy === 'xlsx' ? t('generating') : t('export_excel')}
          </Button>
          <Button variant="outline" size="sm" onClick={() => void handlePdf()} disabled={!!busy}>
          ...
          <Button variant="outline" size="sm" onClick={() => void handleShare()} disabled={!!busy}>
            ...
            {busy === 'share' ? t('generating') : t('share_pdf')}
          </Button>
          <Button variant="outline" size="sm" onClick={() => printDocument(A4)} disabled={!!busy}>
      ...
      <Card>
        <CardHeader className="no-print flex flex-row items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted">{t('preview_hint')}</p>
          </div>
          {loading && <RefreshCw className="h-4 w-4 animate-spin text-muted" strokeWidth={1.7} aria-label={t('loading')} />}
        </CardHeader>
        <CardContent className="print:p-0">
          <div className="overflow-x-auto bg-slate-100 p-3 dark:bg-black/30 print:overflow-visible print:bg-transparent print:p-0">
            <PartnerStatementDocument statement={statement} company={company} filters={filters} />
          </div>
        </CardContent>
      </Card>
    </div>
