...
export default function DeliveryNoteDetailPage() {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      ...
      <Card><CardHeader><CardTitle>{t('documentState')}</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><div className="flex items-center justify-between"><span className="text-muted">{t('status')}</span><Badge tone={statusTone(note.status)}>{statusLabel(note.status)}</Badge></div><div className="flex items-center justify-between"><span className="text-muted">{t('version')}</span><span className="num text-text">{note.version}</span></div>{note.confirmed_at && <div><p className="text-muted">{t('confirmedAt')}</p><p className="num mt-1 text-text" dir="ltr">{formatDateTime(note.confirmed_at, locale)}</p></div>}{note.cancelled_at && <div><p className="text-muted">{t('cancelledAt')}</p><p className="num mt-1 text-text" dir="ltr">{formatDateTime(note.cancelled_at, locale)}</p></div>}</CardContent></Card>
    </div>
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><History className="h-4 w-4 text-primary" strokeWidth={1.7} />{t('history')}</CardTitle></CardHeader><CardContent>{note.events?.length ? <ol className="space-y-4 border-s border-border ps-4">{note.events.map((event) => <li key={event.id} className="relative"><span className="absolute -start-[1.35rem] top-1.5 h-2.5 w-2.5 rounded-full bg-primary" /><div className="flex flex-wrap items-center gap-2"><strong className="text-sm text-text">{eventLabel(event.event)}</strong><span className="text-xs text-muted">{event.actor_name ?? t('systemActor')}</span><time className="num text-xs text-muted" dir="ltr">{formatDateTime(event.occurred_at, locale)}</time></div>{event.reason && <p className="mt-1 text-sm text-muted">{event.reason}</p>}</li>)}</ol> : <p className="text-sm text-muted">{t('noHistory')}</p>}</CardContent></Card>
