...
export default function ProductsPage() {
  ...
  const columns=useMemo<ColumnDef<Product,unknown>[]>(()=>[
    ...
    {accessorKey:'is_active',header:t('status_label'),cell:({row})=><Badge tone={row.original.is_active?'positive':'muted'}>{row.original.is_active?t('active'):t('inactive')}</Badge>},{id:'actions',header:'',cell:({row})=>{const product=row.original,working=workingId===product.id;return <div className="flex items-center justify-end gap-1"><Link href={`/products/${product.id}`}><Button type="button" variant="ghost" size="icon" aria-label={t('view')}><Eye className="h-4 w-4" strokeWidth={1.7}/></Button></Link><Button type="button" variant="ghost" size="icon" aria-label={t('edit')} disabled={working} onClick={()=>{setEditing(product);setDialog(true);}}><Pencil className="h-4 w-4" strokeWidth={1.7}/></Button><Button type="button" variant="ghost" size="icon" aria-label={t('copy')} disabled={working} onClick={()=>void copyProduct(product)}><Copy className="h-4 w-4" strokeWidth={1.7}/></Button><Button type="button" variant="ghost" size="icon" aria-label={t('delete')} disabled={working} onClick={()=>void deleteProduct(product)}><Trash2 className="h-4 w-4 text-negative" strokeWidth={1.7}/></Button></div>;}}
  ],[t,taxInclusive,showStock,workingId]);

  return <div className="space-y-4">
    <div className="flex flex-wrap items-center gap-3"><h1 className="text-xl font-semibold text-text">{t('title')}</h1><div className="ms-auto flex items-center gap-2"><Link href="/products/import"><Button variant="outline"><Upload className="h-4 w-4" strokeWidth={1.7}/>{t('import')}</Button></Link><Link href="/products/new"><Button><Plus className="h-4 w-4" strokeWidth={1.8}/>{t('add')}</Button></Link></div></div>
    <DataExplorerToolbar searchValue={searchInput} searchPlaceholder={`${t('search')} · SKU · ${t('barcode')}`} onSearchChange={setSearchInput} definitions={definitions} filters={labelledFilters} onFilterChange={updateFilter} onRemoveFilter={(key)=>setExplorer(cur=>({...cur,page:1,filters:removeFilter(cur.filters,key)}))} onClearFilters={()=>setExplorer(cur=>({...cur,page:1,filters:[]}))} onOpenAdvanced={()=>setAdvancedOpen(true)}/>
