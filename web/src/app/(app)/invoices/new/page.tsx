  ...
  async function submit(post: boolean) {
    ...
    try {
      const created = await api<{ data: { id: string } }>('/invoices', {
        method: 'POST',
        body: { partner_id: partnerId, payment_type: paymentType, invoice_date: date || null, notes: notes || null, items },
      });
    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {/* رؤوس الأعمدة على الشاشات المتوسطة فأكبر */}
          ...
          {lines.map((l) => {
            ...
            return (
              <div key={l.key} className="grid grid-cols-2 items-center gap-2 rounded border border-border p-2 md:grid-cols-12 md:border-0 md:p-0">
                ...
                <div className="num col-span-1 text-end text-sm md:col-span-1">{formatRiyal((lineTotal + lineTax) / 100)}</div>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <Card className="lg:w-72">
          <CardContent className="space-y-1.5 p-4 text-sm">
            <div className="flex justify-between text-muted">
              ...
              <span className="num">{formatRiyal(subMinor / 100)}</span>
            </div>
            <div className="flex justify-between text-muted">
              <span>{t('tax_total')}</span>
              <span className="num">{formatRiyal(taxMinor / 100)}</span>
            </div>
            <div className="flex justify-between border-t border-border pt-1.5">
              <span className="font-semibold text-text">{t('total')}</span>
              <span className="num text-lg font-bold text-text">{formatRiyal(totalMinor / 100)}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <Button variant="outline" disabled={saving || !partnerId} onClick={() => submit(false)}>
            {t('save_draft')}
          </Button>
          <Button disabled={saving || !partnerId} onClick={() => submit(true)}>
            {t('save_post')}
          </Button>
        </div>
      </div>
