...
export default function FuelAviRfidPage() {
  ...
  async function authorizeFueling() {
    ...
    if (!stationId || !nozzleId || !Number.isSafeInteger(requested) || requested <= 0 || (!vehicleCredential.trim() && !driverCredential.trim())) return;
    setBusy(true); setError(null);
    try {
  ...
  const authorizationColumns = useMemo<ColumnDef<Authorization, unknown>[]>(() => [
    ...
    { accessorKey: 'quantity_milliliters', header: t('quantity'), cell: ({ row }) => <span className="num">{row.original.quantity_milliliters.toLocaleString()}</span> },
