...
export default function FuelStationsMasterDataPage() {
  ...
  function startEdit(row: Item) {
    ...
    Object.keys(next).forEach((key) => { const value = row[key]; next[key] = value === null || value === undefined ? '' : String(value); });
    setForm(next);
  }
  ...
  async function submit(event: React.FormEvent) {
    ...
    Object.entries(form).forEach(([key, value]) => {
      if (['density_kg_per_m3', 'capacity_milliliters', 'safe_capacity_milliliters', 'minimum_level_milliliters', 'dead_stock_milliliters', 'opening_volume_milliliters', 'meter_opening_milliliters'].includes(key)) body[key] = Number(value || 0);
