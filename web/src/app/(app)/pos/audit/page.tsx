...
interface AuditEvent {
  ...
  type: string; category: string | null; amount: string | null; reason_code: string | null; reason_note: string | null;
  source: 'server' | 'client_observed' | 'hybrid' | 'legacy_unknown'; trust_level: string;
  created_at: string | null; payload: Record<string, unknown>; actor?: { id: string; name: string } | null;
  performed_by_user?: { id: string; name: string } | null; approved_by_user?: { id: string; name: string } | null;
...
type AuditTab = 'overview' | 'sensitive' | 'carts' | 'cash' | 'users' | 'approvals' | 'settings';
...
function observedPayload(payload: Record<string, unknown>): Record<string, unknown> {
  ...
  if (clientObserved && typeof clientObserved === 'object' && !Array.isArray(clientObserved)) return clientObserved as Record<string, unknown>;
  const serverObserved = payload.server_observed;
  if (serverObserved && typeof serverObserved === 'object' && !Array.isArray(serverObserved)) return serverObserved as Record<string, unknown>;
  return payload;
}

function eventReference(payload: Record<string, unknown>): string | null {
  const observed = observedPayload(payload);
  for (const key of ['invoice_number', 'invoice_id', 'held_sale_id', 'return_id', 'exchange_id', 'cash_movement_id', 'approval_id']) {
...
export default function PosAuditPage() {
  ...
  const tabs = useMemo<TabDef[]>(() => {
    const next: TabDef[] = [
      { id: 'overview', label: t('overview') }, { id: 'sensitive', label: t('sensitive') },
  ...
  return (
    <main className="space-y-5 p-4 lg:p-6">
      ...
      <TabPanel id={tab}>
        ...
        {tab === 'settings' && <section className="space-y-4"><div className="rounded border border-border bg-surface p-4"><h2 className="text-base font-semibold text-text">{t('settings')}</h2><p className="mt-1 text-sm text-muted">{t('subtitle')}</p><Link href="/pos/settings/configuration" className="mt-3 inline-flex rounded px-1 py-1 text-sm font-medium text-primary underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">{t('settings')}</Link></div><div className="rounded border border-border bg-surface"><div className="border-b border-border p-3"><h2 className="text-base font-semibold text-text">{t('reason')}</h2></div><ul className="divide-y divide-border">{reasonCodes.map((reason) => <li key={reason.id} className="flex items-center justify-between gap-3 p-3"><div><p className="text-sm font-medium text-text">{locale === 'ar' ? reason.name_ar : reason.name_en}</p><p className="num mt-1 text-xs text-muted">{reason.code}{reason.requires_note ? ' · *' : ''}</p></div><span className="text-sm text-muted">{reason.is_active ? tc('active') : tc('inactive')}</span></li>)}</ul></div></section>}
      </TabPanel>

      <Dialog open={selectedEvent !== null} onClose={() => setSelectedEvent(null)} title={t('eventDetails')}>
        {selectedEvent && <div className="space-y-5"><section><h2 className="text-sm font-semibold text-text">{t('humanSummary')}</h2><p className="mt-1 text-sm text-text">{eventLabel(selectedEvent.type)} · {actorName(selectedEvent)} · {formatDate(selectedEvent.created_at, locale)}</p>{selectedEvent.reason_note || selectedEvent.reason_code ? <p className="mt-2 text-sm text-muted">{t('reason')}: {selectedEvent.reason_note ?? selectedEvent.reason_code}</p> : null}</section><section className="grid gap-3 rounded border border-border p-3 sm:grid-cols-2"><div><p className="text-xs text-muted">{t('performedBy')}</p><p className="mt-1 text-sm text-text">{selectedEvent.performed_by_user?.name ?? '—'}</p></div><div><p className="text-xs text-muted">{t('approvedBy')}</p><p className="mt-1 text-sm text-text">{selectedEvent.approved_by_user?.name ?? '—'}</p></div><div><p className="text-xs text-muted">{t('value')}</p><p className="num mt-1 text-sm text-text">{selectedEvent.amount ? formatRiyal(selectedEvent.amount) : '—'}</p></div><div><p className="text-xs text-muted">{t('session')}</p><p className="num mt-1 text-sm text-text">{selectedEvent.session?.number ?? '—'}</p></div></section>{Boolean(observedPayload(selectedEvent.payload).before || observedPayload(selectedEvent.payload).after) && <section className="grid gap-3 sm:grid-cols-2"><div><h2 className="text-sm font-semibold text-text">{t('before')}</h2><pre className="mt-2 max-h-56 overflow-auto rounded border border-border bg-background p-3 text-xs text-text" dir="ltr">{JSON.stringify(observedPayload(selectedEvent.payload).before ?? {}, null, 2)}</pre></div><div><h2 className="text-sm font-semibold text-text">{t('after')}</h2><pre className="mt-2 max-h-56 overflow-auto rounded border border-border bg-background p-3 text-xs text-text" dir="ltr">{JSON.stringify(observedPayload(selectedEvent.payload).after ?? {}, null, 2)}</pre></div></section>}{eventReference(selectedEvent.payload) && <section><h2 className="text-sm font-semibold text-text">{t('references')}</h2><p className="num mt-1 text-sm text-muted" dir="ltr">{eventReference(selectedEvent.payload)}</p></section>}<details><summary className="cursor-pointer text-sm font-medium text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">{t('technicalDetails')}</summary><pre className="mt-2 max-h-56 overflow-auto rounded border border-border bg-background p-3 text-xs text-text" dir="ltr">{JSON.stringify({ id: selectedEvent.id, cart_id: selectedEvent.cart_id, correlation_id: selectedEvent.correlation_id, payload: selectedEvent.payload }, null, 2)}</pre></details></div>}
      </Dialog>
      <Dialog open={cartTimeline !== null} onClose={() => setCartTimeline(null)} title={t('cartTimeline')}>
        {cartTimeline && <div className="space-y-3"><p className="num truncate text-xs text-muted" dir="ltr">{cartTimeline.cartId}</p><ol className="space-y-0 border-s border-border">{cartTimeline.events.map((event) => <li key={event.id} className="relative ps-4 pb-4"><span className="absolute -start-1.5 top-1.5 h-3 w-3 rounded-full border-2 border-surface bg-primary" aria-hidden="true"/><button className="w-full text-start focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40" onClick={() => setSelectedEvent(event)}><p className="text-sm font-medium text-text">{eventLabel(event.type)}</p><p className="mt-1 text-xs text-muted">{formatDate(event.created_at, locale)} · {actorName(event)}</p></button></li>)}</ol></div>}
      </Dialog>
    </main>
  );
}
