  ...
  async function submit() {
    ...
    try {
      const created = await api<{ data: { id: string } }>('/credit-notes', {
        method: 'POST',
        body: { partner_id: partnerId, refund_type: refundType, note_date: date || null, reason: reason || null, items },
      });
      success(tc('created'));
      router.push(`/credit-notes/${created.data.id}`);
    } catch (err) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => router.push('/credit-notes')} aria-label={t('back')}>
