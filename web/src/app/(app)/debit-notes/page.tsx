...
export default function DebitNotesPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    Promise.all([
      api<{ data: CreditNote[] }>(`/credit-notes?type=purchase${branchViewQuery(view, true)}`),
  ...
  useEffect(() => {
    ...
    router.replace(url.toString() ? `/debit-notes?${url.toString()}` : '/debit-notes', { scroll: false });
  }, [explorer, router]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <Select value={explorer.sort ?? '-note_date'} onChange={(event) => setExplorer((current) => ({ ...current, page: 1, sort: event.target.value }))} className="h-9 min-w-44 bg-surface text-sm" aria-label="ترتيب إشعارات المدين">
          <option value="-note_date">الأحدث أولًا</option>
          <option value="note_date">الأقدم أولًا</option>
          <option value="number">الرقم</option>
          <option value="partner">المورد</option>
      ...
      <DataTable columns={columns} data={pageData} loading={loading} emptyLabel={t('empty')} exportName="debit-notes" showToolbar={false} />
