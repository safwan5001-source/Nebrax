...
export default function DocumentsPage() {
  ...
  const load = useCallback(() => { setLoading(true); setError(null); api<{ data: Batch[] }>('/document-batches').then((r) => setBatches(r.data)).catch((e) => setError(e instanceof ApiError ? e.message : t('loadFailed'))).finally(() => setLoading(false)); }, [t]);
  useEffect(() => { load(); }, [load]);
  const visible = useMemo(() => batches.filter((b) => `${b.batch_number ?? b.id} ${b.document_type}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())), [batches, query]);
