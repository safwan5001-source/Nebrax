...
export default function PaymentsPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    Promise.all([
      api<{ data: Payment[] }>(`/payments?direction=received${branchViewQuery(view, true)}`),
  ...
  const methodOptions = useMemo(() => Array.from(new Set(data.map((payment) => payment.method).filter(Boolean)))
    .sort()
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((payment) => {
      if (query) {
        const haystack = [payment.number, partners[payment.partner_id], payment.method]
          .filter(Boolean)
          .join(' ')
          .toLocaleLowerCase();
  ...
  const columns = useMemo<ColumnDef<Payment, unknown>[]>(() => [
    {
      accessorKey: 'number',
      header: t('number'),
      cell: ({ row }) => <Link href={`/payments/${row.original.id}`} className="num font-medium text-primary hover:underline">{row.original.number}</Link>,
    },
  ...
  const sortOptions: SortOption[] = [
    { value: '-payment_date', label: `${t('date')} ↓` },
    { value: 'payment_date', label: `${t('date')} ↑` },
  ...
  const headerActions: PageAction[] = [
    { key: 'create', label: t('create'), icon: Plus, href: '/payments/new', variant: 'primary' },
  ];
