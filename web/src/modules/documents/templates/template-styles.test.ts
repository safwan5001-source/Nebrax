...
describe('أساليب قوالب المستندات الرسمية', () => {
  ...
  it('يميّز مساعدَي الهوية دون خلط التاريخي بـ V2', () => {
    ...
    expect(isLegacyMinimal(MINIMAL_STYLE)).toBe(true);
    expect(isMinimalV2(MINIMAL_STYLE)).toBe(false);
    expect(isMinimalV2(MINIMAL_V2_STYLE)).toBe(true);
    ...
    expect(isQuotationProposal(QUOTATION_PROPOSAL_STYLE)).toBe(true);
    expect(isQuotationProposal(CLASSIC_STYLE)).toBe(false);
    expect(isQuotationProposal(RETAIL_V2_STYLE)).toBe(false);
    expect(QUOTATION_PROPOSAL_STYLE).toMatchObject({
    ...
    expect(QUOTATION_PROPOSAL_STYLE.pagePadding).not.toBe(CLASSIC_STYLE.pagePadding);
    expect(QUOTATION_PROPOSAL_STYLE.pagePadding).not.toBe(MODERN_V2_STYLE.pagePadding);
    expect(QUOTATION_PROPOSAL_STYLE.pagePadding).not.toBe(CLASSIC_V2_STYLE.pagePadding);
