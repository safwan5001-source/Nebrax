...
import { CLASSIC_STYLE, CLASSIC_V2_STYLE, ERP_V2_STYLE, MODERN_V2_STYLE } from '../templates/template-styles';
...
describe('Classic V2 visual composition', () => {
  it('يوسم الغلاف بتركيب classic_v2 لا classic التاريخي', () => {
    const model = makeDocumentQaModel({ scenario: 'five', direction: 'rtl', showQr: true, showAssets: true });
    const { container } = renderComposition(CLASSIC_V2_STYLE, model);
    expect(container.querySelector('[data-doc-composition="classic_v2"]')).toBeTruthy();
  ...
  it('يرسم طرفين بإطار كلاسيكي بلا عمود ميتا ثالث وبلا بطاقة إجمالي زرقاء', () => {
    ...
    expect(container.querySelector('[data-doc-totals="classic_v2"]')?.className).toContain('border-y');
    expect(container.querySelector('[data-doc-totals="classic_v2"]')?.className).not.toContain('rounded-lg');
    expect(container.querySelector('[data-doc-totals="classic_v2"]')?.className).not.toContain('border-s-2');
  });

  it('يرسم QR 70px بجانب الملخص', () => {
    ...
    expect(qr?.getAttribute('width')).toBe(String(CLASSIC_V2.qrSizePx));
  ...
  it('يطوي الملاحظات والبنك والشروط عند الغياب', () => {
    ...
    const { container } = renderComposition(CLASSIC_V2_STYLE, model, FULL_LAYOUT);
  ...
  it('يغطي سيناريوهات 1 و5 و20 بنداً', () => {
    for (const scenario of ['single', 'five', 'twenty'] as const) {
      ...
      const { container, unmount } = renderComposition(CLASSIC_V2_STYLE, model);
  ...
  it('يعرض المبالغ رقمياً ووحدة ريال في الرأس عربياً', () => {
    ...
    expect(container.querySelector('[data-doc-totals="classic_v2"]')?.textContent).toContain('ريال');
  });
  ...
  it('يبقي العربية والإنجليزية المنفردتين بلا وسم عزل', () => {
    ...
    expect(arabic.container.querySelector('[data-doc-totals="classic_v2"]')?.textContent).toContain(modernTotalLabel('subtotal', 'ar'));
    arabic.unmount();
    ...
    expect(english.container.querySelector('[data-doc-totals="classic_v2"]')?.textContent).toContain(modernTotalLabel('grand_total', 'en'));
  });
  ...
  it('لا يغيّر تركيب Classic التاريخي: ثلاث بطاقات وشريط هوية وQR 110', () => {
    ...
    expect(container.querySelector('[data-doc-composition="classic_v2"]')).toBeNull();
    expect(container.querySelector('header .h-14, .h-14')).toBeTruthy();
  ...
  it('لا يحوّل Modern V2 أو ERP V2 إلى تركيب Classic', () => {
    ...
    expect(modern.container.querySelector('[data-doc-composition="classic_v2"]')).toBeNull();
    ...
    expect(erp.container.querySelector('[data-doc-composition="classic_v2"]')).toBeNull();
