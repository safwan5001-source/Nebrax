...
export function DocQr({ model }: { model: DocumentModel }) {
  ...
  const size = isErpV2 ? ERP_V2.qrSizePx : isModernV2 ? VISUAL_V2.qrSizePx : 110;

  return (
    <div className="flex flex-col items-center gap-1">
      {qr ? (
        <>
          <div className={isErpV2 ? 'border border-black p-0.5' : isModernV2 ? 'border border-[color:var(--border)] p-1' : 'rounded-lg border border-gray-200 p-2'}>
