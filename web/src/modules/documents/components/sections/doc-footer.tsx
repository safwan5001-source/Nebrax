...
export function DocFooter({ model }: { model: DocumentModel }) {
  ...
  const usesV2Labels = isModernV2 || isErpV2 || isClassicV2 || style.composition === 'minimal_v2' || style.composition === 'retail_v2' || style.composition === 'quotation_proposal' || style.composition === 'purchase_order_formal';
  const content =
  ...
  const frame = style.composition === 'erp' || isErpV2
    ...
    : isClassicV2
      ? 'border-t border-black pt-3'
      : isModernV2 || isLegacyModern || style.composition === 'minimal_v2' || style.composition === 'retail_v2' || style.composition === 'quotation_proposal' || style.composition === 'purchase_order_formal'
