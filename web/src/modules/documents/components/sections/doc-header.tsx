...
export function DocHeader({ model, showLogo = true }: { model: DocumentModel; showLogo?: boolean }) {
      ...
      <header className="border-b-2 border-black pb-2">
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 flex-1 items-start gap-2">
            ...
            <div className="min-w-0">
              <div className="break-words pt-0.5 text-[13px] font-bold leading-tight text-black">{seller.name || '—'}</div>
              <div className="mt-1 space-y-0">
                ...
                <DocInfoRow label={<ModernFieldLabel field="national_address" mode={mode} />} value={seller.address ? <span className="break-words">{seller.address}</span> : null} stacked />
              </div>
            </div>
          </div>
  ...
  if (style.composition === 'classic_v2') {
    const logoHeight = cappedClassicLogoHeight(seller.logoHeight);
    const invoiceNumberField = isTaxInvoice ? 'number' as const : 'document_number' as const;
    return (
      <header className="border-b border-black pb-3">
        <div className="flex items-start justify-between gap-6">
          <div className="flex min-w-0 flex-1 items-start gap-2.5">
            {showLogo && seller.logoUrl ? (
              ...
              <img
                src={seller.logoUrl}
                alt={seller.name}
                className={CLASSIC_V2.logoMaxWidthClass}
                style={{ height: `${logoHeight}px` }}
              />
            ) : null}
            <div className="min-w-0">
              <div className="break-words pt-0.5 text-[15px] font-bold leading-snug text-black">{seller.name || '—'}</div>
          ...
          <div className="min-w-[172px] max-w-[44%] shrink-0 text-end">
            <h1 className="text-[18px] font-bold leading-tight text-black">
              <ModernDocumentTitle locale={locale} primary={dt(model.type)} alternate={dtAlt(model.type)} mode={mode} />
            </h1>
            <div className="mt-2 space-y-0.5 border-s border-black ps-3">
              <DocInfoRow
                label={<ModernFieldLabel field={invoiceNumberField} mode={mode} />}
                value={<span className="num font-bold" dir="ltr">{meta.number}</span>}
                stacked
            ...
            {isNoticeStatus(model.status) ? (
              <div
                data-doc-status-notice={model.status}
                className={cn(
                  'mt-2 inline-block border border-black px-1.5 py-0.5 text-[10px] font-semibold',
                  model.status === 'cancelled' ? 'text-[color:var(--negative)]' : 'text-black',
                )}
              >
                <ModernStatusLabel status={model.status} mode={mode} />
      ...
      <header className="border-b border-[color:var(--border)] pb-4">
        <div className="flex items-start justify-between gap-8">
          <div className="flex min-w-0 flex-1 items-start gap-3">
            ...
            <div className="min-w-0">
              <div className="break-words pt-0.5 text-[16px] font-bold leading-snug text-black">{seller.name || '—'}</div>
              {seller.tagline ? <div className="mt-0.5 break-words text-[10px] text-[color:var(--muted)]">{seller.tagline}</div> : null}
              <div className="mt-1.5 space-y-0.5">
                <DocInfoRow label={<ModernFieldLabel field="vat_number" mode={mode} />} value={seller.vatNumber ? <span className="num" dir="ltr">{seller.vatNumber}</span> : null} stacked />
                <DocInfoRow label={<ModernFieldLabel field="cr_number" mode={mode} />} value={seller.crNumber ? <span className="num" dir="ltr">{seller.crNumber}</span> : null} stacked />
                <DocInfoRow label={<ModernFieldLabel field="national_address" mode={mode} />} value={seller.address ? <span className="break-words">{seller.address}</span> : null} stacked />
    ...
    return (
      <header className="border-b border-[color:var(--border)] pb-4">
        <div className="flex items-start justify-between gap-8">
          <div className="flex min-w-0 flex-1 items-start gap-3">
            {showLogo && seller.logoUrl ? (
              ...
              <img
                src={seller.logoUrl}
                alt={seller.name}
                className={MINIMAL_V2.logoMaxWidthClass}
                style={{ height: `${logoHeight}px` }}
              />
            ) : null}
            <div className="min-w-0">
              <div className="break-words pt-0.5 text-[17px] font-semibold leading-snug text-black">{seller.name || '—'}</div>
              {seller.tagline ? <div className="mt-1 break-words text-[10px] text-[color:var(--muted)]">{seller.tagline}</div> : null}
          ...
          <div className="min-w-[176px] max-w-[42%] shrink-0 text-end">
            <h1 className="text-[22px] font-semibold leading-tight tracking-tight text-black">
              <ModernDocumentTitle locale={locale} primary={dt(model.type)} alternate={dtAlt(model.type)} mode={mode} />
            </h1>
            ...
            {isNoticeStatus(model.status) ? (
              <div
                data-doc-status-notice={model.status}
                className={cn(
                  'mt-3 inline-block px-0 py-0.5 text-[10px] font-medium',
      ...
      <header className="border-b border-[color:var(--border)] pb-2.5">
        <div className="flex items-start justify-between gap-5">
          <div className="flex min-w-0 flex-1 items-start gap-2">
            ...
            <div className="min-w-0">
              <div className="break-words pt-0.5 text-[14px] font-bold leading-tight text-black">{seller.name || '—'}</div>
              {seller.tagline ? <div className="mt-0.5 break-words text-[9px] text-[color:var(--muted)]">{seller.tagline}</div> : null}
              <div className="mt-1 space-y-0">
                <DocInfoRow label={<ModernFieldLabel field="vat_number" mode={mode} />} value={seller.vatNumber ? <span className="num" dir="ltr">{seller.vatNumber}</span> : null} stacked />
                <DocInfoRow label={<ModernFieldLabel field="cr_number" mode={mode} />} value={seller.crNumber ? <span className="num" dir="ltr">{seller.crNumber}</span> : null} stacked />
                <DocInfoRow label={<ModernFieldLabel field="national_address" mode={mode} />} value={seller.address ? <span className="break-words">{seller.address}</span> : null} stacked />
  ...
  if (style.composition === 'quotation_proposal') {
    const logoHeight = cappedQuotationLogoHeight(seller.logoHeight);
    return (
      <header className="border-b border-[color:var(--border)] pb-4">
        <div className="flex items-start justify-between gap-8">
          <div className="flex min-w-0 flex-1 items-start gap-3">
            {showLogo && seller.logoUrl ? (
              ...
              <img
                src={seller.logoUrl}
                alt={seller.name}
                className={QUOTATION_PROPOSAL.logoMaxWidthClass}
                style={{ height: `${logoHeight}px` }}
              />
            ) : null}
            <div className="min-w-0">
              <div className="break-words pt-0.5 text-[16px] font-semibold leading-snug text-black">{seller.name || '—'}</div>
              {seller.tagline ? <div className="mt-0.5 break-words text-[10px] text-[color:var(--muted)]">{seller.tagline}</div> : null}
              <div className="mt-2 space-y-0.5">
                <DocInfoRow label={<ModernFieldLabel field="vat_number" mode={mode} />} value={seller.vatNumber ? <span className="num" dir="ltr">{seller.vatNumber}</span> : null} stacked />
                <DocInfoRow label={<ModernFieldLabel field="cr_number" mode={mode} />} value={seller.crNumber ? <span className="num" dir="ltr">{seller.crNumber}</span> : null} stacked />
          ...
          <div className="min-w-[176px] max-w-[44%] shrink-0 text-end">
            <h1 className="text-[20px] font-semibold leading-tight tracking-tight text-black">
              <ModernDocumentTitle locale={locale} primary={dt(model.type)} alternate={dtAlt(model.type)} mode={mode} />
            </h1>
            <div className="mt-2 space-y-0.5">
              <DocInfoRow
      ...
      <header className="border-b border-[color:var(--border)] pb-3">
        <div className="flex items-start justify-between gap-6">
          <div className="flex min-w-0 flex-1 items-start gap-2">
            ...
            <div className="min-w-0">
              <div className="break-words pt-0.5 text-[14px] font-semibold leading-snug text-black">{seller.name || '—'}</div>
              {seller.tagline ? <div className="mt-0.5 break-words text-[9px] text-[color:var(--muted)]">{seller.tagline}</div> : null}
              <div className="mt-1.5 space-y-0">
                <DocInfoRow label={<ModernFieldLabel field="vat_number" mode={mode} />} value={seller.vatNumber ? <span className="num" dir="ltr">{seller.vatNumber}</span> : null} stacked />
                <DocInfoRow label={<ModernFieldLabel field="cr_number" mode={mode} />} value={seller.crNumber ? <span className="num" dir="ltr">{seller.crNumber}</span> : null} stacked />
