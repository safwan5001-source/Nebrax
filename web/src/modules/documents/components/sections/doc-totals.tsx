...
import { PURCHASE_ORDER_FORMAL } from '../../presentation/purchase-order-formal';
...
export function DocTotals({
  ...
  const baseRow = cn(
    ...
    isPurchaseOrderFormal && 'items-start px-0 py-1 text-black text-[10px]',
  );

  const outer = cn(
    isModernV2 ? VISUAL_V2.totalsMaxClass : isErpV2 ? ERP_V2.totalsMaxClass : isClassicV2 ? CLASSIC_V2.totalsMaxClass : isMinimalV2 ? MINIMAL_V2.totalsMaxClass : isRetailV2 ? RETAIL_V2.totalsMaxClass : isQuotationProposal ? QUOTATION_PROPOSAL.totalsMaxClass : isPurchaseOrderFormal ? PURCHASE_ORDER_FORMAL.totalsMaxClass : 'max-w-[300px]',
    !usesV2Labels && 'overflow-hidden',
    isModernV2 || isLegacyModern || isErpV2 || isClassicV2 || isMinimalV2 || isRetailV2 || isQuotationProposal || isPurchaseOrderFormal ? 'w-full' : 'w-[46%]',
    ...
    isPurchaseOrderFormal && 'border-y border-[color:var(--border)]',
    isLegacyModern && 'rounded-md border border-[color:var(--border)] bg-white',
    isMinimal && 'border-y border-black',
    !isErp && !isModernV2 && !isErpV2 && !isClassicV2 && !isMinimalV2 && !isRetailV2 && !isQuotationProposal && !isPurchaseOrderFormal && !isLegacyModern && !isMinimal && 'rounded-lg border border-gray-200',
  );
  ...
  const totalRow = cn(
    ...
    isPurchaseOrderFormal && 'items-start border-t-2 border-[color:var(--border)] px-0 py-1.5 text-black text-[12px] font-semibold',
    isLegacyModern && 'border-t-2 border-[color:var(--doc-brand)]',
    isMinimal && 'border-t-2 border-black px-0',
  );
  const totalStyle = isMinimal || isModernV2 || isErpV2 || isClassicV2 || isMinimalV2 || isRetailV2 || isQuotationProposal || isPurchaseOrderFormal
    ? undefined
    : isLegacyModern
      ? { background: 'var(--doc-brand-soft)', color: 'var(--doc-brand)' }
      : { background: 'var(--doc-brand)', color: 'var(--doc-brand-contrast)' };

  const showTaxRow = !(isQuotationProposal || isPurchaseOrderFormal) || (totals.tax != null && totals.tax > 0);

  return (
    <div className={outer} data-doc-totals={style.composition}>
      <div className={cn('flex items-baseline justify-between gap-5 px-3 py-1.5', isErp || isMinimal || isModernV2 || isErpV2 || isClassicV2 || isMinimalV2 || isRetailV2 || isQuotationProposal || isPurchaseOrderFormal ? 'text-black' : 'text-[color:var(--muted)]', (isMinimal || isModernV2 || isMinimalV2) && 'px-0 py-2', isModernV2 && 'items-start', isErpV2 && 'items-start px-0 py-1 text-[10px]', isClassicV2 && 'items-start px-0 py-1.5 text-[11px]', isMinimalV2 && 'items-start', isRetailV2 && 'items-start px-0 py-1', isQuotationProposal && 'items-start px-0 py-1.5 text-[11px]', isPurchaseOrderFormal && 'items-start px-0 py-1 text-[10px]')}>
