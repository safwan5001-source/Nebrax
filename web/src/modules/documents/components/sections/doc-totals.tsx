...
export function DocTotals({
  ...
  const outer = cn(
    isModernV2 ? VISUAL_V2.totalsMaxClass : isErpV2 ? ERP_V2.totalsMaxClass : 'max-w-[300px]',
    !usesV2Labels && 'overflow-hidden',
    isModernV2 || isLegacyModern || isErpV2 ? 'w-full' : 'w-[46%]',
    ...
    !isErp && !isModernV2 && !isErpV2 && !isLegacyModern && !isMinimal && 'rounded-lg border border-gray-200',
  );
  ...
  const totalStyle = isMinimal || isModernV2 || isErpV2
    ? undefined
    : isLegacyModern
  ...
  return (
    <div className={outer} data-doc-totals={style.composition}>
      <div className={cn('flex items-baseline justify-between gap-5 px-3 py-1.5', isErp || isMinimal || isModernV2 || isErpV2 ? 'text-black' : 'text-[color:var(--muted)]', (isMinimal || isModernV2) && 'px-0 py-2', isModernV2 && 'items-start', isErpV2 && 'items-start px-0 py-1 text-[10px]')}>
