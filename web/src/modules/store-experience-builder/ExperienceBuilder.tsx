...
import { ControlPanels, CUSTOMIZER_PANELS, type CustomizerPanel } from "./ControlPanels";
...
export type BuilderLifecycle = "clean" | "dirty" | "save_blocked" | "publish_blocked";
    ...
    >
      <header className="flex h-12 shrink-0 items-center gap-2 border-b border-neutral-300 bg-white px-3">
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold leading-none">{t("title")}</p>
            ...
            {(["desktop", "tablet", "mobile"] as const).map((item) => (
              <button
                ...
                className={`h-7 px-2 text-[11px] font-medium ${
                  device === item ? "bg-neutral-900 text-white" : "text-neutral-700"
                }`}
              >
                {t(item)}
              </button>
          ...
          <div className="flex rounded border border-neutral-300 p-0.5">
            {(["ar", "en"] as const).map((item) => (
              <button
                ...
                className={`h-7 px-2 text-[11px] font-medium ${
                  locale === item ? "bg-neutral-900 text-white" : "text-neutral-700"
                }`}
              >
      ...
      {notice ? (
        ...
        >
          ...
          {notice}
          {VERSION_HISTORY_CAPABILITY === "deferred" ? ` ${t("versionDeferred")}` : null}
        </div>
      ) : null}
