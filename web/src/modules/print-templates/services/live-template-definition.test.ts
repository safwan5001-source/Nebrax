...
describe('محول تعريف القالب الحي', () => {
  ...
  it('يحافظ على هوية tax-invoice-erp-v2 دون سقوط إلى التاريخي أو classic', () => {
    const resolved = resolveLiveTemplateDefinition({
      print_template_revision_id: 'erp-v2-revision',
      revision: {
        id: 'erp-v2-revision',
        definition: { template_id: 'tax-invoice-erp-v2', footer_text: 'تذييل ERP V2' },
      },
    }, 'tax_invoice');

    expect(resolved?.templateId).toBe('tax-invoice-erp-v2');
  ...
  it('يحافظ على هوية tax-invoice-minimal-v2 دون سقوط إلى التاريخي', () => {
    const resolved = resolveLiveTemplateDefinition({
      print_template_revision_id: 'minimal-v2-revision',
      revision: {
        id: 'minimal-v2-revision',
        definition: { template_id: 'tax-invoice-minimal-v2', footer_text: 'تذييل Minimal V2' },
      },
    }, 'tax_invoice');

    expect(resolved?.templateId).toBe('tax-invoice-minimal-v2');
