...
describe('ContextualHelpTrigger', () => {
  ...
  it('opens the same contextual sheet from the mobile user-menu action and restores focus', async () => {
    function MenuHarness() {
      ...
      return (
        <>
          <button ref={accountButtonRef} type="button">الحساب</button>
          <ContextualHelpTrigger placement="menu" restoreFocusRef={accountButtonRef} />
        </>
      );
    }

    renderIntl(<MenuHarness />);
    ...
    await waitFor(() => {
      expect(document.activeElement).toBe(screen.getByRole('button', { name: 'الحساب' }));
    });
  });
