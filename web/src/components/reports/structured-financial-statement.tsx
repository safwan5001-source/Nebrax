...
function ValueCell({ value, strong = false }: { value?: FinancialStatementValue; strong?: boolean }) {
  return (
    <TD className={`num px-3 py-2 text-end tabular-nums ${strong ? 'font-semibold' : ''} ${amountClassName(value)}`}>
      {value?.amount === undefined ? '—' : formatRiyal(value.amount)}
    </TD>
  );
}
...
function StatementDesktopRow({ row, columns }: { row: FinancialStatementRow; columns: FinancialStatementColumn[] }) {
  ...
  return (
    <TR className={isSubtotal ? 'border-t border-border bg-background font-semibold hover:bg-background' : ''}>
      <TH scope="row" className={`px-3 py-2 text-start font-medium ${isSubtotal ? 'text-text' : 'text-text'}`}>
        {rowLabel(row)}
      </TH>
      {values.map((value, index) => <ValueCell key={columns[index].id} value={value} strong={isSubtotal} />)}
    </TR>
  );
}
      ...
      {columns.map((column, index) => {
        ...
        return (
          <div key={column.id} className="min-w-0">
            ...
            <dd className={`num mt-1 text-end tabular-nums ${strong ? 'font-semibold' : 'font-medium'} ${amountClassName(value)}`}>
              {value?.amount === undefined ? '—' : formatRiyal(value.amount)}
            </dd>
          </div>
        );
...
function StatementMobileRow({ row, columns }: { row: FinancialStatementRow; columns: FinancialStatementColumn[] }) {
  ...
  if (isEmpty) {
    return <li className="flex min-h-10 items-center justify-center px-3 py-2.5 text-center text-sm text-muted">{rowLabel(row)}</li>;
  }
  ...
  return (
    <li className={`flex min-h-10 items-center justify-between gap-4 px-3 py-2.5 text-sm ${isSubtotal ? 'border-t border-border bg-background font-semibold text-text' : ''}`}>
      <div className="min-w-0">{rowLabel(row)}</div>
      <span className={`num shrink-0 text-end tabular-nums ${isSubtotal ? 'font-semibold' : 'font-medium'} ${amountClassName(value)}`}>
        {value?.amount === undefined ? '—' : formatRiyal(value.amount)}
      </span>
    </li>
  );
...
function GrandTotal({ row, columns, mobile = false }: { row: FinancialStatementRow; columns: FinancialStatementColumn[]; mobile?: boolean }) {
  ...
  if (mobile) {
    if (isMultiValue) {
      return (
        <div className="mt-3 rounded border border-primary/20 bg-primary-soft px-3 py-3 text-sm">
          <span className="font-semibold text-text">{row.label}</span>
          <MobileValues columns={columns} values={values} strong />
        </div>
      );
    }
    const value = values[0];
    return (
      <div className="mt-3 flex min-h-12 items-center justify-between gap-4 rounded border border-primary/20 bg-primary-soft px-3 py-3 text-sm">
        <span className="min-w-0 font-semibold text-text">{row.label}</span>
        <span className={`num shrink-0 text-end font-semibold tabular-nums ${amountClassName(value)}`}>{value?.amount === undefined ? '—' : formatRiyal(value.amount)}</span>
      </div>
    );
  }
  ...
  return (
    <TR className="border-t-2 border-primary/20 bg-primary-soft font-semibold text-text hover:bg-primary-soft">
      ...
      {values.map((value, index) => <ValueCell key={columns[index].id} value={value} strong />)}
    </TR>
  );
}
...
function Equation({ row, columns, mobile = false }: { row: FinancialStatementRow; columns: FinancialStatementColumn[]; mobile?: boolean }) {
  ...
  if (mobile) {
    if (isMultiValue) {
      return <div className="mt-2 border-t border-border px-3 py-2.5 text-xs"><span className="text-muted">{row.label}</span><MobileValues columns={columns} values={values} /></div>;
    }
    const value = values[0];
    return <div className="mt-2 flex min-h-10 items-center justify-between gap-4 border-t border-border px-3 py-2.5 text-xs"><span className="min-w-0 text-muted">{row.label}</span><span className={`num shrink-0 text-end tabular-nums ${amountClassName(value)}`}>{value?.amount === undefined ? '—' : formatRiyal(value.amount)}</span></div>;
  }
  ...
  return (
    <TR className="border-t border-border text-sm hover:bg-transparent">
      ...
      {values.map((value, index) => <ValueCell key={columns[index].id} value={value} />)}
    </TR>
  );
}
 ...
 * أو تخصيص أعمدة. تُستخدم `columns` اختيارياً فقط للبيانات المالية متعددة القيم
 * التي توفرها قائمة التدفقات النقدية الحالية؛ ليست Report Builder عاماً.
 */
export function StructuredFinancialStatement({
  ...
  const valueColumns = columns?.length ? columns : [{ id: 'amount', label: amountLabel ?? '' }];
        ...
        {sections.map((section) => (
          <section key={section.id} aria-labelledby={`statement-section-${section.id}`}>
            ...
            <ul className="divide-y divide-border rounded border border-border bg-surface" aria-label={section.label}>
              {section.rows.map((row) => <StatementMobileRow key={row.id} row={row} columns={valueColumns} />)}
            </ul>
          </section>
        ))}
        ...
        <Table>
          <THead>
            <TR className="hover:bg-transparent">
              ...
              {valueColumns.map((column) => <TH key={column.id} scope="col" className="text-end">{column.label}</TH>)}
            </TR>
          </THead>
          {sections.map((section) => (
            <TBody key={section.id} aria-label={section.label}>
              <TR className="bg-background hover:bg-background">
                <TH scope="rowgroup" colSpan={valueColumns.length + 1} className="px-3 py-2 text-start text-xs font-semibold text-muted">{section.label}</TH>
              </TR>
              {section.rows.map((row) => <StatementDesktopRow key={row.id} row={row} columns={valueColumns} />)}
            </TBody>
          ))}
          {(grandTotal || equation) && (
            <tfoot>
              {grandTotal && <GrandTotal row={grandTotal} columns={valueColumns} />}
              {equation && <Equation row={equation} columns={valueColumns} />}
            </tfoot>
          )}
        </Table>
      </div>
