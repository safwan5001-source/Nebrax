...
/**
 * نموذج إنشاء الإشعار — مشترك بين الدائن (للعميل) والمدين (للمورّد).
 * النوع يحدّد: فضاء الترجمة · نوع الطرف المعروض · جسم الطلب · وجهة العودة.
 */
export function CreditNoteForm({ type = 'sales' }: { type?: 'sales' | 'purchase' }) {
  const isPurchase = type === 'purchase';
  const t = useTranslations(isPurchase ? 'debitNotes' : 'creditNotes');
  ...
  useEffect(() => {
    ...
    api<{ data: Partner[] }>(isPurchase ? '/partners?type=supplier' : '/partners?type=customer').then((r) => {
      setPartners(r.data);
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const created = await api<{ data: { id: string } }>('/credit-notes', {
        method: 'POST',
        body: { type, partner_id: partnerId, refund_type: refundType, note_date: date || null, reason: reason || null, items },
      });
      success(tc('created'));
      router.push(`/credit-notes/${created.data.id}`); // شاشة التفاصيل مشتركة للنوعين
    } catch (err) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => router.push(isPurchase ? '/debit-notes' : '/credit-notes')} aria-label={t('back')}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="refund">{t('refund_type')}</Label>
              <Select id="refund" value={refundType} onChange={(e) => setRefundType(e.target.value)}>
