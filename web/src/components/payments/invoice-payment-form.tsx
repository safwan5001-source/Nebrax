...
export function InvoicePaymentForm({ invoiceId }: { invoiceId: string }) {
  ...
  const remainingMinor = riyalToMinor(invoice?.remaining ?? '0');
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const [invoiceResponse, partnersResponse, methodsResponse, accountsResponse, collectorsResponse] = await Promise.all([
          api<{ data: Invoice }>(`/invoices/${invoiceId}`),
        ...
        const loadedInvoice = invoiceResponse.data;
        if (loadedInvoice.status !== 'posted' || loadedInvoice.payment_status === 'paid') {
          setError(t('load_invoice_payment_failed'));
          return;
        }
        ...
        setCustomerName(partnersResponse.data.find((partner) => partner.id === loadedInvoice.partner_id)?.name ?? '—');
        setMethods(activeMethods);
        setAccounts(accountsResponse.data);
        setCollectors(collectorsResponse.data);
        setAmount(loadedInvoice.remaining);
        ...
        if (!cancelled) setError(requestError instanceof ApiError ? requestError.message : t('load_invoice_payment_failed'));
      } finally {
  ...
  async function submit(mode: SubmitMode) {
    if (!invoice || amountMinor <= 0 || amountMinor > remainingMinor || !paymentMethodId || !cashAccountId) {
    ...
    body.append('partner_id', invoice.partner_id);
    body.append('invoice_id', invoice.id);
    body.append('direction', 'received');
    ...
    try {
      ...
      if (mode === 'post') {
        ...
        success(t('confirm_collection_success'));
        router.push(`/invoices/${invoice.id}`);
      } else {
        success(t('save_draft_success'));
        router.push(`/receipt-vouchers/${created.data.id}`);
      }
    } catch (requestError) {
  ...
  if (loading) {
    return <p className="text-sm text-muted">{t('loading_invoice_payment')}</p>;
  }

  if (!invoice) {
    return <p className="rounded-md bg-negative/10 px-4 py-3 text-sm text-negative">{error ?? t('load_invoice_payment_failed')}</p>;
  }
    ...
    <div className="mx-auto max-w-5xl space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <Button variant="ghost" size="icon" onClick={() => router.push(`/invoices/${invoice.id}`)} aria-label={t('back')}>
            <ArrowRight className="h-4 w-4" strokeWidth={1.7} />
          </Button>
          <div>
            <h1 className="text-xl font-semibold text-text">{t('invoice_payment_title')}</h1>
            <p className="mt-1 text-sm text-muted">{t('invoice_payment_subtitle')}</p>
          </div>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="space-y-5">
          <Card>
            ...
            <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="invoice-payment-amount">{t('amount')}</Label>
                <div className="flex rounded-md border border-input bg-background focus-within:ring-2 focus-within:ring-primary/40">
                  <Input id="invoice-payment-amount" inputMode="decimal" className="num border-0 text-end shadow-none focus-visible:ring-0" value={amount} onChange={(event) => setAmount(event.target.value)} required />
                  <span className="flex items-center border-s border-input px-3 font-medium text-muted" dir="ltr">SAR</span>
                </div>
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="invoice-payment-date">{t('date')}</Label>
                <Input id="invoice-payment-date" type="date" dir="ltr" value={date} onChange={(event) => setDate(event.target.value)} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="invoice-payment-method">{t('method')}</Label>
                <Select id="invoice-payment-method" value={paymentMethodId} onChange={(event) => selectMethod(event.target.value)} required>
                  <option value="" disabled>{t('method')}</option>
                  {methods.map((method) => <option key={method.id} value={method.id}>{method.name}</option>)}
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="invoice-payment-account">{t('cash_account')}</Label>
                <Select id="invoice-payment-account" value={cashAccountId} onChange={(event) => setCashAccountId(event.target.value)} required disabled={!selectedMethod}>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="invoice-payment-collector">{t('collector')}</Label>
                <Select id="invoice-payment-collector" value={collectorEmployeeId} onChange={(event) => setCollectorEmployeeId(event.target.value)}>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="invoice-payment-reference">{t('reference')}</Label>
                <Input id="invoice-payment-reference" value={reference} onChange={(event) => setReference(event.target.value)} placeholder={t('reference_hint')} />
              </div>
              <div className="space-y-1.5 sm:col-span-2">
                <Label htmlFor="invoice-payment-details">{t('payment_details')}</Label>
                <textarea id="invoice-payment-details" rows={3} value={paymentDetails} onChange={(event) => setPaymentDetails(event.target.value)} placeholder={t('payment_details_placeholder')} className="min-h-24 w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-text outline-none transition focus-visible:ring-2 focus-visible:ring-primary/40" />
              </div>
              <div className="space-y-1.5 sm:col-span-2">
                <Label htmlFor="invoice-payment-notes">{t('receipt_notes')}</Label>
                <textarea id="invoice-payment-notes" rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder={t('receipt_notes_placeholder')} className="min-h-24 w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-text outline-none transition focus-visible:ring-2 focus-visible:ring-primary/40" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <label htmlFor="invoice-payment-attachments" className="flex min-h-28 cursor-pointer flex-col items-center justify-center gap-2 rounded-md border border-dashed border-border bg-muted/40 px-4 text-center transition hover:border-primary/50 focus-within:ring-2 focus-within:ring-primary/40">
                ...
                <Input id="invoice-payment-attachments" type="file" multiple className="sr-only" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.jpg,.jpeg,.png,.gif,.zip" onChange={(event) => addFiles(event.target.files)} />
              </label>
        ...
        <aside className="space-y-5">
          <Card>
            <CardHeader><CardTitle>{t('invoice_summary')}</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              <div className="flex items-start gap-2 text-sm text-text"><FileText className="mt-0.5 h-4 w-4 shrink-0 text-muted" strokeWidth={1.7} /><span className="num">{invoice.number}</span></div>
              <div><p className="text-xs text-muted">{t('customer')}</p><p className="mt-1 text-sm font-medium text-text">{customerName}</p></div>
              <div><p className="text-xs text-muted">{t('remaining')}</p><p className="num mt-1 text-base font-semibold text-text">{formatRiyal(invoice.remaining)}</p></div>
              <p className="border-t border-border pt-3 text-xs leading-5 text-muted">{t('invoice_payment_only')}</p>
            </CardContent>
          </Card>
        </aside>
      </div>
      ...
      <div className="flex flex-wrap justify-end gap-2">
        <Button type="button" variant="outline" onClick={() => router.push(`/invoices/${invoice.id}`)} disabled={saving !== null}>{t('back')}</Button>
        ...
        <Button type="button" onClick={() => submit('post')} disabled={saving !== null}>{saving === 'post' ? t('posting') : t('confirm_collection')}</Button>
      </div>
    </div>
  );
}
