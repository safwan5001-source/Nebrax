...
/**
 * إنشاء استرداد مورّد: المال يعود فعلاً من المورّد مقابل مرتجعات مشتريات
 * مرحّلة. الاسترداد **مخصَّصٌ بالكامل** — مجموع التخصيصات = المبلغ، ولا
 * تخصيص يتجاوز الرصيد القابل للاسترداد على مرتجعه. الأرقام تأتي محسوبة من
 * الخادم (`eligible-returns`)، فلا تحسبها الواجهة بنفسها.
 */
export function CreateSupplierRefundDialog({
  open,
  onClose,
  onCreated,
}: {
  ...
  const [eligible, setEligible] = useState<EligibleReturn[]>([]);
  ...
  // مرتجعات المورّد المؤهّلة — بأرصدتها القابلة للاسترداد كما يحسبها الخادم.
  useEffect(() => {
    setEligible([]);
    setAllocations({});
    if (!open || !partnerId) return;
    api<{ data: EligibleReturn[] }>(`/supplier-refunds/eligible-returns/${partnerId}`)
  ...
  function setAllocation(returnId: string, value: string) {
    setAllocations((current) => ({ ...current, [returnId]: value }));
  }

  function fill(row: EligibleReturn) {
    setAllocation(row.id, String(row.refundable / 100));
  }

  async function submit() {
    setError(null);
    const rows = Object.entries(allocations)
      .filter(([, value]) => value !== '' && riyalToMinor(value) > 0)
      .map(([purchaseReturnId, value]) => ({
    ...
    setSaving(true);
    try {
      const created = await api<{ data: { id: string } }>('/supplier-refunds', {
        method: 'POST',
        body: {
          partner_id: partnerId,
          // المبلغ = مجموع التخصيصات دائماً: لا استرداد جزئي التخصيص في V1.
      ...
      if (postNow) await api(`/supplier-refunds/${created.data.id}/post`, { method: 'POST' });
      success(tc('created'));
      setAllocations({});
      setReference('');
      setNotes('');
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            <Label htmlFor="srf-partner">{t('supplier')}</Label>
            <Select id="srf-partner" value={partnerId} onChange={(event) => setPartnerId(event.target.value)}>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="srf-method">{t('destination')}</Label>
            <Select id="srf-method" value={paymentMethodId} onChange={(event) => setPaymentMethodId(event.target.value)}>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="srf-date">{t('refund_date')}</Label>
            <Input id="srf-date" type="date" value={refundDate} onChange={(event) => setRefundDate(event.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="srf-reference">{t('reference')}</Label>
            <Input id="srf-reference" value={reference} onChange={(event) => setReference(event.target.value)} />
          </div>
        </div>
                  ...
                  {eligible.map((row) => (
                    <tr key={row.id} className="border-b border-border/70 last:border-0">
                      <td className="px-2 py-2">
                        ...
                        <span className="num mx-2 text-xs text-muted">{row.return_date}</span>
                      </td>
        ...
        <div className="space-y-1.5">
          <Label htmlFor="srf-notes">{t('notes')}</Label>
          <Textarea id="srf-notes" value={notes} onChange={(event) => setNotes(event.target.value)} />
        </div>
