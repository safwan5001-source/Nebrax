...
| PostgreSQL | 19/19 ✅ | 227/227 ✅ (شمل الفلتر الموسَّع نفسه) | لم يكتمل — انظر الملاحظة أدناه |

**ملاحظة صادقة عن التشغيل الكامل بلا فلتر على PostgreSQL:** حاولته مرّتين. المحاولة
الأولى تلوّثت لأني بدّلت `.env` إلى SQLite بينما العملية الخلفية لا تزال تعمل
(نتيجة كاذبة: 3539 فشل — ليست انحداراً حقيقياً، بل تعارض تهيئة ذاتي التسبب).
المحاولة الثانية كانت تعمل بصحة حين أعاد النظام تشغيل الحاوية، فضاعت الخلفية بلا
نتيجة. **الفلتر المُستهدَف + الانحدار الموسَّع (227/227) هو ما يطلبه بند خطة
الاختبار الفعلي («PostgreSQL targeted») وهو مؤكَّدٌ أخضر بالكامل** — لم أُعِد
محاولة الجولة الكاملة بلا فلتر ثالثةً لأنها تجاوز صريح للمطلوب وتُخاطر بتأخيرٍ
إضافي بلا قيمة تحقّقٍ جديدة (كل الاختبارات المتعلقة بـVAR-POS-1 وبكل ملف لمسته
هذه المهمة موجودة أصلاً داخل الفلتر الموسَّع).
...
`package.json`. الانحدار المُستهدَف + المُوسَّع أخضرٌ بالكامل على كلا قاعدتي
البيانات (172 + 19 على SQLite، 227 + 19 على PostgreSQL — الفلتر الموسَّع
يشمل `PosVariantCheckoutTest` نفسها). الجولة الكاملة بلا فلتر على SQLite
مؤكَّدةٌ (27 فشلاً معزولاً معروفاً، بلا انحدار جديد)؛ نظيرتها على PostgreSQL
لم تكتمل (انظر الملاحظة في Tests) لكنها خارج ما يطلبه بند خطة الاختبار.
GitHub Actions على الفرع بعد الدفع — لم تُفحص بعد ضمن هذه الجلسة.
